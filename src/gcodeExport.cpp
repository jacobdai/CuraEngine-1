...
void GCodeExport::writeArc(Point p, int speed, int lineWidth,int r,int clk,Point pcenter)
{
    ...
    int clockarc=(INT2MM(pcenter.X)-INT2MM(pcur.X))*(INT2MM(p.Y)-INT2MM(pcenter.Y))-(INT2MM(pcenter.Y)-INT2MM(pcur.Y))*(INT2MM(p.X)-INT2MM(pcenter.X));
	...
	if(clk>0)
	    fprintf(f, "G03 F%i X%0.3f Y%0.3f R%i %c%0.5f pcur.x%0.3f pcur.y%0.3f pcen.x%0.3f pcen.y%0.3f clock%i\n",speed * 60,INT2MM(p.X - extruderOffset[extruderNr].X), INT2MM(p.Y - extruderOffset[extruderNr].Y),rarc, extruderCharacter[extruderNr], extrusionAmount, INT2MM(pcur.X - extruderOffset[extruderNr].X), INT2MM(pcur.Y - extruderOffset[extruderNr].Y), INT2MM(pcenter.X - extruderOffset[extruderNr].X), INT2MM(pcenter.Y - extruderOffset[extruderNr].Y), clockarc);
	if(clk<0)
