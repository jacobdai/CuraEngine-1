...
void GCodeExport::writeArc(Point p, int speed, int lineWidth,float ra,float coli,float colj,int clk,Point pcenter)
{
    ...
    Point pcuro;
    pcuro.X=2*pcenter.X-pcur.X;
    pcuro.Y=2*pcenter.Y-pcur.Y;
    long long int clockarc=(INT2MM(p.X)-INT2MM(pcur.X))*(INT2MM(pcuro.Y)-INT2MM(p.Y))-(INT2MM(p.Y)-INT2MM(pcur.Y))*(INT2MM(pcuro.X)-INT2MM(p.X));
	...
	if(clk<0)
	    fprintf(f, "G02 F%i X%0.3f Y%0.3f I%0.3f J%0.3f %c%0.5f\n",speed * 60,INT2MM(p.X - extruderOffset[extruderNr].X), INT2MM(p.Y - extruderOffset[extruderNr].Y), coli, colj, extruderCharacter[extruderNr], extrusionAmount);
...
void GCodeExport::writeMove(Point p, int speed, int lineWidth)
{
	...
	float ynext=p.Y - extruderOffset[extruderNr].Y;
        ...
        {
            Point diff = p - getPositionXY();
            fprintf(f, "currentPosition.x%0.3f    currentPosition.y%0.3f", INT2MM(currentPosition.x - extruderOffset[extruderNr].X),INT2MM(currentPosition.y - extruderOffset[extruderNr].Y));
            if (isRetracted)
            {
                if (retractionZHop > 0)
                    fprintf(f, "G1 Z%0.3f\n", float(currentPosition.z)/1000);
	...
	if((((xpos-xnext)>100000.0)||((xpos-xnext)<-100000.0)||((ypos-ynext)>100000.0)||((ypos-ynext)<-100000.0))&&(lineWidth == 0))
	{
	  ...
	  }else
	  {
		int zadd=currentPosition.z+10000;
                ...
                fprintf(f, "G0 F%i X%0.3f Y%0.3f  %c%0.5f\n", speed * 60 ,INT2MM(p.X - extruderOffset[extruderNr].X), INT2MM(p.Y - extruderOffset[extruderNr].Y),extruderCharacter[extruderNr], extrusionAmount);
                fprintf(f, "G0 F%i Z%0.3f  %c%0.5f", speed * 60 , INT2MM(zadd-10000),extruderCharacter[extruderNr], extrusionAmount);
	  }
          fprintf(f, "\n");
	}else
	  {
    ...
    for(unsigned int n=0; n<paths.size(); n++)
    {
            ...
            if (path->points.size() == 1 && path->config != &travelConfig && shorterThen(gcode.getPositionXY() - path->points[0], path->config->lineWidth * 2))
        {
            ...
            if (i > n + 2)
            {
                ...
                for(unsigned int x=n+1; x<i-1; x+=2)
                {
